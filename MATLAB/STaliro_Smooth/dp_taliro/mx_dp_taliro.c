...
common1:		sdump(n->lft,dumpbuf);
	...
	case NEXT:	strcat(dumpbuf, "X");
			goto common1;
	case NOT:	strcat(dumpbuf, "!Miscell");
			goto common1;
...
{	
    ...
    /* to see smooth rob is being used*/
    mxArray *array_ptr;
    array_ptr = mexGetVariable("global", "taliro_SmoothRob");
    
    if(array_ptr== NULL) /*no smoothness flag*/
    {
        smooth_operator = 0; /*this is shared in DynamicProgramming*/
    }
    else
    {        
    double *mex_val;
    mex_val = (double *)mxGetPr(array_ptr);
    smooth_operator = *mex_val;
    /*mexPrintf("%s%g\n", "Using smooth criminals (1-yes,0-No): ",smooth_operator);*/
    }
