...
% (C) 2015, Adel Dokhanchi, Arizona State University
	...
	properties
        ...
        % 
		%   Default option: opt.ode_solver = 'default';
		%
		...
		% 
		%   Default option : opt.falsification = 1;
		%
		...
		%
		%   Default option : opt.interpolationtype = {'pchip'};
		%
		...
		%
		%   Default option : opt.SampTime = .05;
		%
		...
		%
		%   Default option : opt.black_box = 0;
		%
		...
		%
		%   Default option : opt.spec_space = 'Y';
		%
		...
		%
		%   Default option : opt.loc_traj = 'none';
		%
		...
		%
		%   Default option : opt.SimulinkSingleOutput = 0;
		%
		...
		%
		%   Default option : opt.save_intermediate_results = 0;
		%
		...
		%
		%   Default option : opt.save_intermediate_results_varname = 'default';
		% 
		...
		%
		%   Default option : opt.taliro_metric = 'none';
		%
		...
		%
		%   Default option : opt.rob_scale = 100;
		%   
		...
		%
		%   Default option : opt.RobustnessOffset = [];
		%
		...
		%
		%   Default option : opt.taliro = 'dp_taliro';
		%   
		...
		%
		%   Default option : opt.n_workers = 1;
		%
		...
		%
		%   Default option : opt.stochastic = 0;
		%
		...
		%
		%   Default option : opt.hasim_params = [1 0 0 0 0];
		% 
		...
		%
		%   Default option : opt.optimization = 'min';
		%
		%   Options : 'min' or 'max'
		%
		...
		%
		%   Default option : opt.parameterEstimation = 0;
		%
		...
		%
		%   Default option : opt.dp_t_taliro_direction = 'both';
		%  
		...
		%  
		%   Default option : opt.seed = 0;
		%
        %   This option sets the seed for the random number generator. Specifically, it sets
        %   the seed for the random number stream 'mt19937ar'. The seed should be a value 
        ...
        % Choose which dimension of the state or output space should be considered for specification robustenss.
		%
		%   Default option : opt.dim_proj = [];
		% 
		...
		%
		%   Default option : opt.taliro_undersampling_factor = 1;
		%
		...
		%
		%   Default option : opt.varying_cp_times = 0; 
		%
		...
		% 
        %   Currently, the feature is only supported for Simulated Annealing optimization engines due to the complexity 
		%   of the search space.
        varying_cp_times = 0;
        
        % Select whether to normalize the temporal logic robustness values in the interval [0 1].
		%
		%   Default option : opt.normalization = 0;
		%
		...
		%
		%   Default option : opt.polarity_weight = 'norm';
		%
		...
		%
		%   Default option : opt.param_est_fals_stage = 1;
		%
		...
		%
		%   Default option : opt.fals_at_zero = 1;
		%
		...
		% Define constrained input spaces (experimental option)
		% 
		%    Default option : opt.input_space_constrained = 0;
		%    
		%    If you want to define a convex search space different from a hypercube,
		%    then set the property input_space_constrained to 1. In this case,		
		%    you can define a search space of the form Ax<=b using the properties
		%    input_A and input_b in the class staliro_options. If the set of initial  
        %    conditions in staliro is also defined, then the search space will be the 
        %    intersection of the initial hypercube and the set Ax<=b.	
		input_space_constrained = 0;
		...
		%
		%   Default option : opt.customCostAuxData = [];
		%
