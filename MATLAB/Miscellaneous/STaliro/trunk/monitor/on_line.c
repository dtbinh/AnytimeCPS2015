...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    MTLformula = (mxArray*)ssGetSFcnParam(S,0);
    ObservationMap = (mxArray*)ssGetSFcnParam(S,1);
    SampleTime = (mxArray*)ssGetSFcnParam(S,2);
    delta = (double)mxGetScalar(SampleTime);
    if(delta==0){
        ssPrintf("Error! Sample time should be more than 0\n");
        return;
    }
...
static void mdlInitializeSampleTimes(SimStruct *S)
{
    ...
    int_T  systemDimention;
     ...
     systemDimention    = ssGetInputPortWidth(S,0);
     fw_taliro_param.SysDim=systemDimention;
     XTrace=(double*)malloc(sizeof(double)*systemDimention);
...
static void mdlStart(SimStruct *S)
{
    ...
    /* Get the formula */
   	ndim = mxGetNumberOfDimensions(MTLformula);
	dims = mxGetDimensions(MTLformula);
	...
	status = mxGetString(MTLformula, uform, buflen);   
    ...
    /*---------------------------------------------------------------------------------*/
    /* Get predicate map*/
    NElems = mxGetNumberOfElements(ObservationMap);
	...
	{
        /* Get predicate name */
        tmp = mxGetField(ObservationMap, jstruct, "str");
	    if(tmp == NULL) 
		{
        ...
        /* Get set */
        tmp = mxGetField(ObservationMap, jstruct, "A");
	    /* If A is empty, then we should have an interval */
		if(tmp == NULL)
		{ 
            tmp = mxGetField(ObservationMap, jstruct, "Int");
			if(tmp == NULL) {
				mexPrintf("%s%s \n", "Predicate: ", predMap[jstruct].str);
				mexPrintf("Error: Above predicate: Both fields 'A' and 'Int' do not exist!"); 
                sprintf(msg,"%s%s \n Error: Above predicate: Both fields 'A' and 'Int' do not exist!", "Predicate: ", predMap[jstruct].str);
                ssSetErrorStatus(S,msg);
                return;
			}
	    }
		...
		{ 
			...
			if (ndimA>2)
			{
                ...
                sprintf(msg,"%s%s \nError: Above predicate: The set constraints are not in proper form!", "Predicate: ", predMap[jstruct].str);
                ssSetErrorStatus(S,msg);
                return;
			}
			...
			if (fw_taliro_param.SysDim != dimsA[1])
			{
                ...
                sprintf(msg,"%s%s \nError: Above predicate: The dimensions of the set constraints and the state trace do not match!", "Predicate: ", predMap[jstruct].str);
                ssSetErrorStatus(S,msg);
                return;
			}
			...
			/* get b */
			tmp = mxGetField(ObservationMap, jstruct, "b");
			if(tmp == NULL) 
			{ 
				mexPrintf("%s%s\n", "Predicate: ", predMap[jstruct].str);
				mexErrMsgTxt("Error: Above predicate: Field 'b' is empty!"); 
                sprintf(msg,"%s%s\nError: Above predicate: Field 'b' is empty!", "Predicate: ", predMap[jstruct].str);
                ssSetErrorStatus(S,msg);
                return;
			}
			...
			if (predMap[jstruct].set.ncon != dimsb[0])
			{
                ...
                sprintf(msg,"%s%s\nError: Above predicate: The number of constraints between A and b do not match!", "Predicate: ", predMap[jstruct].str);
                ssSetErrorStatus(S,msg);
                return;
			}
			...
			if (predMap[jstruct].set.ncon==2 && fw_taliro_param.SysDim==1)
			{
					  ...
					  (predMap[jstruct].set.A[1][0]<0 && (predMap[jstruct].set.A[1][0]/predMap[jstruct].set.b[1]<=predMap[jstruct].set.A[0][0]/predMap[jstruct].set.b[0]))))
				{
                    ...
                    sprintf(msg,"%s%s\nError: Above predicate: The constraint is the empty set!", "Predicate: ", predMap[jstruct].str);
                    ssSetErrorStatus(S,msg);
                    return;
				}
			}
	...
	/*-----BFS for formula--------------*/
	phi_size=BreadthFirstTraversal(Q,misc->node,qi);
    
    misc->subformula=(Node**)emalloc(sizeof(Node*)*(phi_size+1));
    
    computeHorizon(misc,misc->node);
    FH = misc->node->FiniteHorizon;
    History = misc->node->History+FH;/*Hst=Hrz+hst(\phi)*/
    
 	ssPrintf("The size of the formula is %d   Finite Horizon=%d   History=%d  \n",phi_size,FH,History);

    misc->RobTable=(HyDis**)emalloc(sizeof(HyDis*)*(phi_size+1));
    misc->pre=(HyDis*)emalloc(sizeof(HyDis)*(phi_size+1));
    for(ii=0;ii<=phi_size;ii++)
        misc->RobTable[ii]=(HyDis*)emalloc(sizeof(HyDis)*(FH+History+1));
    for(ii=1;ii<=phi_size;ii++){
...
void  computeHorizon(Miscellaneous *misc,Node *phi){
    ...
    case WEAKNEXT:
        if( isItZero2Inf(phi->time) ){
            ...
            phi->History = left_history;
            //ssPrintf("CORRECT: Interval for next/prev operator is zero to infinity \n");
        }
        else
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    /*TStamps*/
    InputRealPtrsType vPtrs = ssGetInputPortRealSignalPtrs(S,1);
    ...
    timeStamp=(*vPtrs[0]);
	SampleNum++;
    rob=FH_Taliro(misc,predMap,XTrace,SampleNum,LTrace,&distData,&fw_taliro_param);
    for (i=0; i<width; i++) {
        /*
         * This example does not implement complex signal handling.
         * To find out see an example about how to handle complex signal in 
...
void printTable(Miscellaneous *misc){
 ...
 HyDis FH_Taliro(Miscellaneous *misc, PMap *predMap, double *XTrace, int i, double *LTrace, DistCompData *p_distData, FWTaliroParam *p_par){
     ...
     iHst=History;
     if(i>(History)){/*  pre variable should be updated pre <- now */
          for(k=1;k<=phi_size;k++){
     ...
     for(k=phi_size;k>=1;k--){
         if(misc->subformula[k]->past<1){
             for(j=(FH+History);j>=misc->subformula[k]->History+(History-iHst);j--)
                  UpdateRob(misc,k,j,XTrace,p_distData,p_par); 

		 }
         else{
             for(j=misc->subformula[k]->History+(History-iHst);j<=(FH+History);j++)
                UpdateRobPast(misc,k,j,XTrace,i,p_distData,p_par); 

         }
     }
     if(Prediction==0&&Globally==0){
         robustness = misc->RobTable[1][History];
     }
       		...
       		case PREV:
                 ...
                 if( misc->subformula[k]->Lindex!=0 ){
                     ...
                     if(iterDown>=0){
                         ...
                         ssPrintf("Error: UpdateRob for PREV: iterDown<0 semantically impossible!\n");
					}
       		...
       		case WEAKPREV:
                ...
                if( misc->subformula[k]->Lindex!=0 ){
                     ...
                     if(iterDown>=0){
                         ...
                         ssPrintf("Error: UpdateRob for WEAKPREV: iterDown<0 semantically impossible!\n");
					}
                     misc->RobTable[k][evalTime]=max;
                 }else{
      		...
      		case EVENTUALLY_PAST:
                if( isItAny2Inf(misc->subformula[k]->time) ){
                    if( misc->subformula[k]->Rindex!=0 ){
                        ...
                        else{/* from History-iHst to eval*/
                            iterDown = (History)-iHst;/*begining of simulation (index)*/
                            for( n=iterUp ; n>=iterDown  ; n-- ){
                    ...
                    if( misc->subformula[k]->Rindex!=0 ){
                        ...
                        if(iterDown>=0){
                            ...
                            ssPrintf("Error: UpdateRobPast for EVENTUALLY_PAST: iterDown<0 semantically impossible!\n");
						}
                        misc->RobTable[k][evalTime]=max;
                    }else{
                        ssPrintf("Error: UpdateRobPast for EVENTUALLY_PAST: right index is not valid!\n");
                ...
                if( isItAny2Inf(misc->subformula[k]->time) ){
                    ...
                    if( misc->subformula[k]->Rindex!=0 && misc->subformula[k]->Lindex!=0 ){
                        ...
                        if(iterDown>=0){
                            ...
                            ssPrintf("Error: UpdateRobPast for SINCE: iterDown<0 semantically impossible!\n");
						}
                        misc->RobTable[k][evalTime]=max;
                    }else{
                        ssPrintf("Error: UpdateRobPast for SINCE: right/left index is not valid!\n");
                 ...
                 if( isItAny2Inf(misc->subformula[k]->time) ){
                    ...
                    if( misc->subformula[k]->Rindex!=0 && misc->subformula[k]->Lindex!=0 ){
                        ...
                        if(iterDown>=0){
                            ...
                            ssPrintf("Error: UpdateRobPast for DELAY: iterDown<0 semantically impossible!\n");
						}
                        misc->RobTable[k][evalTime]=min;
                    }else{
                 ...
                 if( isItAny2Inf(misc->subformula[k]->time) ){
                    ...
                    if( misc->subformula[k]->Rindex!=0 ){
                        ...
                        if(iterDown>=0){
                            ...
                            ssPrintf("Error: UpdateRobPast for ALWAYS_PAST: iterDown<0 semantically impossible!\n");
						}
                        misc->RobTable[k][evalTime]=min;
                    }else{
                        ssPrintf("Error: UpdateRobPast for ALWAYS_PAST: right index is not valid!\n");
...
void UpdateRob(Miscellaneous *misc,int k,int horizon, double *xx, DistCompData *p_distData, FWTaliroParam *p_par){
    ...
    int   n,iterBegin,iterEnd;
    		...
    		case VALUE:
    		case PREDICATE:
                if( horizon>=History && horizon<=(FH+History)  ){
            ...
            case OR:
       		...
       		case WEAKNEXT:
				...
				break;
       		case EVENTUALLY:
                if( ( horizon + misc->subformula[k]->LBound ) <= (FH+History) ){
                    max=SetToInf(-1);
                    iterBegin = horizon+misc->subformula[k]->LBound;
                    if( misc->subformula[k]->Rindex!=0){
                        if(horizon + misc->subformula[k]->UBound < (FH+History) ) 
	...
	switch (phi->ntyp)
	{
		...
		case PREDICATE:
			...
			if (!phi->sym->set)
			{
				...
				mexErrMsgTxt("Error: The set for the above predicate has not been defined!\n");
			}
		...
		default:
			mexPrintf("%s%s\n", "Node: ", phi->ntyp);
			mexErrMsgTxt("Error: The above operator is not supported!\n");
	}
	return(phi);
}
...
HyDis monitor(Node *phi, PMap *predMap, double *XTrace, double TStamp, double *LTrace, DistCompData *p_distData, FWTaliroParam *p_par)
{
	...
	for (ii=0;ii<p_par->nPred;ii++)
	{
		...
		phi2= dupnode (phi);
		/* Progress formula */
		phi = ProgRob(phi,XTrace,p_distData,&ct,&dt,&last,p_par);
	...
	/* Last timestamp */
		ct.numf.f_num = t_cur;
	phi2 = ProgRob(phi2,XTrace,p_distData,&ct,&dt,&last,p_par);
	if (phi2->ntyp==TRUE)
	{
		robustness=SetToInf(+1);
	}
	else if (phi2->ntyp==FALSE)
	{
		robustness=SetToInf(+1);
	}
	else 
	{
		/* assertion */
		if (phi2->ntyp!=VALUE)
			ssPrintf("fw_taliro ERROR: There is a problem! This line should not have been reached! \n");
		else
			robustness=phi2->rob;
	}
    releasenode(1,phi2);
