function [hs, rc] = systemsimulator(inputModel, XPoint, UPoint, staliro_SimulationTime, staliro_InputBounds, nb_ControlPoints)
...
% SYNOPSYS
%    
%     [hs, rc] = systemsimulator(inputModel, XPoint, UPoint, staliro_SimulationTime, staliro_InputBounds, nb_ControlPoints)
%     
% DESCRIPTION
% 
...
if isempty(UPoint)
    ...
    InpSignal = ComputeInputSignals(steptime, UPoint,staliro_opt.interpolationtype, nb_ControlPoints, staliro_InputBounds, staliro_SimulationTime);
...
if strcmp(inputModelType, 'function_handle')
        ...
        [T, XT] = SimFcnPtr(ode_solver, inputModel, staliro_SimulationTime, XPoint, UPoint, staliro_InputBounds, nb_ControlPoints, staliro_opt.interpolationtype);        
...
elseif strcmp(inputModelType, 'blackbox_model')
         ...
         if ~isa(inputModel.model_fcnptr,'function_handle')
             error('S-Taliro: the balckbox model must be an function pointer')
         end
         ...
         if (isempty(CLG) && isempty(GRD))
             ...
             if ~( isempty(inputModel.CLG) && isempty(inputModel.Grd))
                 if ~(isequal(CLG,inputModel.CLG)&& isequal(GRD,inputModel.Grd))
                     error('S-Taliro: Mismatch between staliro_blackbox and blackbox model in setting CLG/Grd. Please use either stalior_blackbox or blackbox model to set CLG and Grd')
                 end 
             else
                 inputModel.CLG = CLG ;
                 inputModel.Grd = GRD;
...
elseif strcmp(inputModelType, 'hautomaton')  
    ...
    if isfield(inputModel, 'simulator')
        ...
        [ht, ~, rc] = mysimulator(inputModel, h0, staliro_SimulationTime, ode_solver, staliro_opt.hasim_params);
    end    
    T =  ht(:, 2); % get time
