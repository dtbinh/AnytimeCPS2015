 ...
 %built in double switch blocks check for '>' or '>=' guard can be directly written with '-1'.           
    ...
    for i_count=1:switch_count
        if(Switchconnhandlebool(i_count,1)~=0)
            ...
            Operatortype=get_param(Switchconnhandlebool(i_count,1),'Operator');
            if(~strcmpi(Relsourcehandle(i_count,1).source1type,'Constant'))
                if (strcmpi(Operatortype,'<'))
                    if projection_mode == 0
                        c(Cntrlgraphindex).grd(1,2).A(1,Relsourcehandle(i_count,1).out1)=1;
                    else
                        c(Cntrlgraphindex).grd(1,2).A(1,i_proj)=1;
                        c(Cntrlgraphindex).grd(1,2).proj(1,i_proj)=Relsourcehandle(i_count,1).out1;
                        i_proj=i_proj+1;
                    end
                else
                    if projection_mode == 0
            ...
            if(~strcmpi(Relsourcehandle(i_count,1).source2type,'Constant'))
                if (strcmpi(Operatortype,'<'))
                    if projection_mode == 0
                        c(Cntrlgraphindex).grd(1,2).A(1,Relsourcehandle(i_count,1).out2)=-1;
                    else
                        c(Cntrlgraphindex).grd(1,2).A(1,i_proj) = -1;
                        c(Cntrlgraphindex).grd(1,2).proj(1,i_proj) = Relsourcehandle(i_count,1).out2;
                        i_proj=i_proj+1;
                    end
                else 
                    if projection_mode == 0
                ...
                if (strcmpi(Operatortype,'<'))
                    if(isnan(str2double(get_param(Relsourcehandle(i_count,1).source2,'Value'))))
                        try
                            strvalue=get_param(Relsourcehandle(i_count,1).source2,'Value');
                            c(Cntrlgraphindex).grd(1,2).b=-1*evalin('base',strvalue);
                        catch
                            error('Unable to evaluate the block value.Please load the corresponding variable file in base workspace.')
                        end
                    else
                        c(Cntrlgraphindex).grd(1,2).b=str2double(get_param(Relsourcehandle(i_count,1).source2,'Value'));
                    end
                    ...
                    if(isnan(str2double(get_param(Relsourcehandle(i_count,1).source2,'Value'))))
                        ...
                        c(Cntrlgraphindex).grd(1,2).b=-1*str2double(get_param(Relsourcehandle(i_count,1).source2,'Value'));
                    end
                end
            end
            if projection_mode == 0
