function [model_instr_info] = model_instrumentation(modelName,projection_mode,exclusion_list)%,filename_string)%instrument_mode)
...
% check for input correctness
    if (nargin == 0)
              fprintf('\n USAGE: model_instrumentation(''modelName'')\n');
    ...
    blocklist_struct= Model_Reader(modelName,exclusion_list); % basic model annotation and blocklist generation
    ...
    switch_block_info = Switch_Instrument(modelName,blocklist_struct.switch_blocks,blocklist_struct.blockList,blocklist_struct.connHandleList); % Instruments switch blocks in the model and stores relevent info
