...
% S-TaLiRo
%		
%		Systems' TemporAl LogIc RObustness : A toolbox to perform Temporal Logic 
%		Falsification and Parameter mining for Cyber-Physical Systems (models, 
%		software or hardware in the loop)
%
% USAGE:
% [results, history] =
%       staliro(model,init_cond,input_range,cp_array,phi,preds,TotSimTime,opt)
%
% DESCRIPTION :
%       S-Taliro performs temporal logic falsification and parameter mining for 
%	    hybrid systems models. The input model can be in several forms, such as 
%		a Simulink model or an m-function, and the specification can be a 
%       Signal Temporal Logic (STL) formula which is encoded using a Metric 
%       Temporal Logic (MTL) formula.
%
% INPUTS :
%   - model : can be of type:
%       * function handle : it represents a pointer to a function which will be
%		  numerically integrated using an ode solver (default solver: ode45;  
%		  the solver can be changed through staliro_options), or a
%		* Blackbox class object : the user will provide the system behavior 
%		  based on given inputs (see staliro_blackbox help file), or
% 		* string : it is the name of the Simulink model to be simulated.
%
%       Examples: 
...
%		Important: 
%
%		For black box setup see the demos (under demos folder):
...
%       using the staliro option <a href="matlab: doc staliro_options.input_space_constrained">staliro_options.input_space_constrained</a>. 
%
%   - input_range : 
%       The constraints for the parameterization of the input signal space.
%       The following options are supported:tic;sum(B(:)==preds(1).A(:))==nos*noc;toc
%
...
%       In case of:
%
%		* Signals generated using interpolation between the control points, 
%		  e.g., piece-wise linear or splines (for more options see 
%		  staliro_options.interpolationtype): 
%		  
...
%					staliro_options.varying_cp_times
...
%
% See also : staliro_options, SimSimulinkMdl, SimFunctionMdl, SimBlackBoxMdl,
% 			 staliro_blackbox, dp_taliro, fw_taliro, dp_t_taliro, polarity
...
if (~isreal(init_cond) || (~isreal(input_range) &&  ~iscell(input_range)) )
...
% Process and handle input ranges and control points
...
[m,dum] = size(input_range); %#ok<ASGLU,NASGU>
...
[m1,dum] = size(input_range); %#ok<ASGLU,NASGU>
n2 = length(cp_array);
if(m1~=n2)
...
if opt.varying_cp_times > 0
    ...
    elseif opt.varying_cp_times == 2
        ...
        if staliro_opt.input_space_constrained == 1
            % Resize arrays to take into account the new search variables
            staliro_opt.input_A = [staliro_opt.input_A zeros(size(staliro_opt.input_A,1),temp_ControlPoints(end)-2*n2)];
        else
            staliro_opt.input_space_constrained = 1; % Make sure the constrained space search option is enabled
            staliro_opt.input_A = [];
            staliro_opt.input_b = [];
        end
        n_var_tmp = temp_ControlPoints(end); 
        for j = 1:n2
            ...
            if cp_array(j)>3
                ...
                bc_tmp = ones(tot_idx_tmp, 1)*(-(1e-6)*staliro_SimulationTime); % The interpolation functions require the time points to be different
                ...
                bc_tmp(tot_idx_tmp+2) = staliro_SimulationTime*(1-(1e-6));                
                staliro_opt.input_A = [staliro_opt.input_A; Ac_tmp];
                staliro_opt.input_b = [staliro_opt.input_b; bc_tmp];
            end
