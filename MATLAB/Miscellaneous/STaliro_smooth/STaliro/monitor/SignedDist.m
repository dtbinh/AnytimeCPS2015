...
if(~smooth)
    ...
    if(scalar) %use scalar techniques
        ...
        for i = 1:numel(SmoothOpt.preds.Sets)  % check which predicate is this motherfucker
            if ((sum(SmoothOpt.preds.Sets(i).A(:)==A(:))==nos*noc) && (sum(SmoothOpt.preds.Sets(i).b(:)==b(:))==noc)) %talking about this predicate here g
                if(~exist('alt_getWavApprox_vector_genable_mex')) %if no mex
                    dist =  alt_getWavApprox_vector_genable(x,SmoothOpt.preds.WavParams(i).C_00k, SmoothOpt.preds.WavParams(i).D_ejk,...
                        SmoothOpt.preds.WavParams(i).k_min,SmoothOpt.preds.WavParams(i).k_max,SmoothOpt.preds.WavParams(i).j_min,SmoothOpt.preds.WavParams(i).j_max, ...
                        SmoothOpt.preds.WavParams(i).E_dash);%multvar approx here
                else %call mex
                    dist =  alt_getWavApprox_vector_genable_mex(x,SmoothOpt.preds.WavParams(i).C_00k, SmoothOpt.preds.WavParams(i).D_ejk,...
                        SmoothOpt.preds.WavParams(i).k_min,SmoothOpt.preds.WavParams(i).k_max,SmoothOpt.preds.WavParams(i).j_min,SmoothOpt.preds.WavParams(i).j_max, ...
                        SmoothOpt.preds.WavParams(i).E_dash);%multvar approx here
