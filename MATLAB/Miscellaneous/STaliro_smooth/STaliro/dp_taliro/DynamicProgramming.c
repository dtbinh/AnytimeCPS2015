...
/* Foundation, Inc., 59 Temple Place, Suite 330, Boston, MA 02111-1307 USA*/getWavApprox
...
extern double smooth_operator; /*from mx_dp_taliro.c*/
/*
...
{
    /*needs overflow check on smooth*/
    if(smooth_operator == 0) /*no smootheness*/
    {
        ...
        if ((inp1.dl<inp2.dl) || ((inp1.dl==inp2.dl) && (inp1.ds<=inp2.ds)))
        {
            /*debug purposes*/
            /*int is_inf; 
            is_inf = (isinf(inp1.ds)||isinf(inp2.ds));
            if(is_inf==1)
            {
                mexPrintf("%s%d\n", "inf test max ",is_inf);
                mexPrintf("%s%f%s%f%s%f\n", "Hizzle: ",inp1.ds," ",inp2.ds," ",inp2.ds);
            }
            */
            return(inp2);
        }
        else
        {
            ...
            {
                mexPrintf("%s%d\n", "inf test max ",is_inf);
                mexPrintf("%s%f%s%f%s%f\n", "Hizzle: ",inp1.ds," ",inp2.ds," ",inp1.ds);
            }*/
            return(inp1);
        }
    }
        else /*smooth max*/
        {
            ...
            /* declare and init output here */
            HyDis HDistOut;
            HDistOut.dl = 1;
            HDistOut.iteration = 0;
            HDistOut.preindex = 0;
            
            /*smooth max*/
            HDistOut.ds = (1/C)*log(exp(C*inp1.ds)+exp(C*inp2.ds));
            /*mexPrintf("%s%f%s%f%s%f%s%d\n", "Hizzle: ",inp1.ds," ",inp2.ds," ",HDistOut.ds," is inf ",isinf(HDistOut.ds));*/
            /* if isinf(smoothmax) && ~(isinf(both input arguments)
             * while(isinf(smoothmax)
             *C = C/10;compute smoothmax
             */
            
            /*int is_inf;
            is_inf = isinf(HDistOut.ds);
            if(is_inf==1)
            {
                mexPrintf("%s%d\n", "inf test smax",is_inf);
                mexPrintf("%s%f%s%f%s%f\n", "Hizzle: ",inp1.ds," ",inp2.ds," ",HDistOut.ds);
            }
            */
            while( !(isinf(inp1.ds) && isinf(inp2.ds)) && isinf(HDistOut.ds) && (C>=1))/*if either of inpts is inf and output is*/
            {
                C = C/10;
               /* mexPrintf("%s\n","Decimating");*/
                HDistOut.ds = (1/C)*log(exp(C*inp1.ds)+exp(C*inp2.ds));
                /*mexPrintf("%s%f%s%f%s%f%s%f\n", "Hizzle_dec: ",inp1.ds," ",inp2.ds," ",HDistOut.ds," C: ",C);*/
            }
            return(HDistOut);
        }
}
...
HyDis hmin(HyDis inp1, HyDis inp2)
    {
        /*needs underflow check on smooth*/
        if(smooth_operator == 0) /*no smootheness*/
        {
            if ((inp1.dl<inp2.dl) || ((inp1.dl==inp2.dl) && (inp1.ds<=inp2.ds)))
            {
        ...
        else /*yeah is smooth*/
        {
            ...
            HDistOut.ds = (-1/C)*log(exp(-C*inp1.ds)+exp(-C*inp2.ds));
            /*int is_inf;
            is_inf = (isinf(HDistOut.ds) || isinf(inp1.ds) || isinf(inp2.ds));
            if(is_inf==1)
            {
                mexPrintf("%s%d\n", "inf test smin",is_inf);
                mexPrintf("%s%f%s%f%s%f\n", "Hizzle: ",inp1.ds," ",inp2.ds," ",HDistOut.ds);
            }*/
            while( !(isinf(inp1.ds) && isinf(inp2.ds)) && isinf(HDistOut.ds) && (C>=1)) /*if either of inpts is inf, and output is*/
            {
