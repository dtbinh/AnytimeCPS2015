classdef Quad_ccta
    ...
    methods
        % constructor
        function obj = Quad_ccta(x,y,z,yaw,pitch,roll)
            obj.isFollowing = 0;
            obj.x = x;
            obj.y = y;
            obj.z = z;
        ...
        function outs = drawBody(obj)
            diameter = 0.05; % 5 cm
            [X,Y,Z] = sphere();
            X = X*diameter + obj.x;
            Y = Y*diameter + obj.y;
            Z = Z*diameter + obj.z;
            outs.p = surf(X,Y,Z,'EdgeColor','none','FaceColor',[0 0 1]);
            % shadow
            outs.q = plot(obj.x,obj.y,'.','LineWidth',2,...
                'MarkerEdgeColor','k',...
                'MarkerFaceColor','k',...
                'MarkerSize',10);
        end
        % draw arms
        function outs = drawArms(obj)
            r = 0.02; % 2 cm radius
            n = 10; % granularity
