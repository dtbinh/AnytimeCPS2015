...
\end{equation}
The input $ u_k$ is constrained to be in $[-0.52,0.52]^2 \forall k$. 
...
We first run BluSTL in satisfaction mode, with the control cost (norm of $u$) to 0.
To emulate a satisfaction mode in Smooth Operator we simply stop the optimization iterations when the robustness exceeds the approximation error $\varepsilon$.
