...
In \cite{Raman14_MPCSTL}, the authors encoded the MTL formula as a set of linear and boolean constraints, and used Gurobi to solve the resulting Mixed Integer Linear Program (MILP).
MILPs are NP-hard.
The sophisticated heuristics used to mitigate this typically yield local optima, and make it hard to characterize their runtimes, which is important in control - see examples in \cite{Raman14_MPCSTL} and this paper.
%Like the method of this paper, in general, MILP solvers yield local optima, which is a reflection of the non-convexity of the objective, but our method, being faster, allows for a larger number of initializations in a multi-start fashion.
