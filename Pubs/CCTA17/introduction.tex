...
\ypcomment{Contributions of this paper: make them explicit in a list if possible. Move this to after related work}
...
MILPs are NP-complete, inherently non-convex, and do not scale well with increasing number of variables.  In general, MILP solvers can only guarantee achieving local optima. The sophisticated heuristics used to mitigate these make it hard to characterize their runtimes, which is important in control - see examples in \cite{Raman14_MPCSTL}. The implementation of the algorithm in \cite{Raman14_MPCSTL} can be found in the tool BluSTL \cite{}, and is used for comparison with our approach throughout this paper.
