...
To emulate mode (B), we terminate the optimization as soon as $\srob \geq \epsilon_{\text{Meyer}}$, which in turn implies that $\rob_{\formula} \geq 0$.
For SOP we also set $\delta=0$ in $P_{\srob}$ for all cases.
