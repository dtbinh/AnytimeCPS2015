...
\small
\begin{center}
\caption{{\small Runtimes (mean and standard, in seconds) for Smooth Operator (SOP), BluSTL (BlS) and Simulated annealing (SA) over 100 runs with random initial states. Here, (B) means \textit{boolean} mode and (R) means \textit{robust} mode of operation. BlS (R) runtimes are not reported since it does not find trajectories that satisfy the specification. See text for more details.}}
\vspace{-5pt}
