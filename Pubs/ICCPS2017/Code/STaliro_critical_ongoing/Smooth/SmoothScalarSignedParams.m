function wavparams = SmoothScalarSignedParams(lb,ub,xmin,xmax,dx)

%mapping between -20 and 20, check if that works
...
if(exist('Staliro')>0)
    tic
    dist_array_x = arrayfun(@(x) SignedDist_noSmooth(x,P.A,P.b), grid_x);
    toc
    figure;
