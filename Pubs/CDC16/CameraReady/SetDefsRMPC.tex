...
Mapping sets between $z$ and $x$ spaces via the arbitrary diffeomorphism $T$ has to be done numerically, and we show how in the \ifthenelse{\boolean{TECH_REPORT}}{appendix.} {online report \cite{PantAM16_RMPC}.}
...
For $1 \leq j \leq N$, we define the $j$-step over-approximate reach set computed at time $k$ to be
 ...
 This construction of $\oaXset{k+j}{k}$ permits us to prove recursive feasibility of the RMPC controller, because it causes the constraints of the RMPC problem setup at time $k+1$ to be consistent with the constraints of the problem setup at time $k$.
...
We also define the set $\tE_{max}$, which is necessary for the terminal constraints of Eq. \eqref{eq:P_f_def}. $\tilde{E}_{max}$ represents the worst case bound on the estimation error $\te_{k}$, and is computed similar to Eq. \eqref{eq:H}, but over the entire set $X$. %and not reachable subsets of it:
