...
Therefore the linearization error $h_{k+j|k} + r_{k+j|k}$ is bounded in the set $H_{k+j|k} + R_{k+j|k}$. 
In the experiments we ignore the remainder term $R_{k+j|k}$ based on the observation that the error $e_k$ is small relative to the state $x_k$. Thus we use:
\begin{eqnarray}
\label{eq:err_lin}
\tE_{k+j|k} = H_{j+k|j}
\end{eqnarray}
