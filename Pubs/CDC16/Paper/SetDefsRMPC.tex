...
\[\ua{V}(\Xc) \defeq \{v=[v_1,\ldots,v_{\dimV}]^T \such \max_{x\in \Xc} \ua{v,U}_i(x)  \leq v_i \leq \min_{x \in \Xc} \oa{v,U}_i(x) \} \]
\[\oa{V}(\Xc) \defeq \{v=[v_1,\ldots,v_{\dimV}]^T \such \min_{x\in \Xc} \ua{v,U}_i(x)  \leq v_i \leq \max_{x \in \Xc} \oa{v,U}_i(x) \} \]
...
\begin{figure}
	...
	\caption{Illustration of the local and global inner approximations of input constraints for running example, with an arbitrary $\oa{X}_{k+j|k} =  [-\pi/4,0]\times[-0.9666,-0.6283]$ for some $k,j$ and $U = [-2.75,2.75]$. Color in online version.}
...
\begin{figure}
	...
	\caption{The error sets $\tilde{E}_{max}$ and $\tilde{E}$ computed over an arbitrary $\oa{X}_{k+j|k}$ as defined in the text. Also shown are random realizations of the error $\tilde{e}$ for random $e \in E$ and $x \in \chi$.}
