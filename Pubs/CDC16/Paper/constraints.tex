...
\todo[inline]{Yash these following equations: should that really be $\hat{L}_N\hat{F}$, or just $\hat{F}$? Also, why the added $p \in P_N(\tilde{E}_{max}, \tilde{E}_{max})$? we already said that "if p in $C_p$"}
By definition of the robust control invariant set $C_p$, there exists a feedback control law $v=\hat{K}p$, such that $\forall p\in C_p$
\begin{subequations}
\begin{align}
\label{eq:C_def}
