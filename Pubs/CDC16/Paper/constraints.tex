...
V_{k+j|k} &= \underline{V}_{k+j|k} \ominus_{i=0}^{j-1}KL_iW_{k+(j-i)|k} 
\end{align}
\end{subequations}
...
\begin{equation}
P_f = C_p\ominus \hat{L}_N\hat{F}{W}_{max}
\end{equation}

Here, $C_p$ is a robust control invariant set for the worst case disturbances given by $W_{max}$. Note. $C_p$ is the invariant set inside $P_N(\tilde{E}_{max},\tilde{E}_{max})$, where $\hat{Z}_N(\tilde{E}_{max},\tilde{E}_{max})$ is computed recursively, similar to Eq. \ref{eq:Set_constraints} but with all error sets replaced by $\tilde{E}_{max}$ and correspondingly all disturbance sets by $W_{max}$, as follows $\forall j=0,\dotsc,N$
...
P_{j} &= P \ominus_{i=0}^{j-1}(\hat{L}_i\hat{F}W_{max})\ominus (-\hat{F}\tilde{E}_{max})
\end{align}
\end{subequations}
