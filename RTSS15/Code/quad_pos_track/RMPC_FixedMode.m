...
for t = 1:Tsim+1
    ...
    for i = 1:numel(deltas)
        ...
        [vars{i},status{i}] = cvxsolve_30(params{i},[]);
         figure(i);
         hold on;grid on;
       % for j = 1:30
        %    plot(t+j-1,vars{i}.u{j}(1),'ro');hold on;
         %   plot(t+j-1,vars{i}.u{j}(2),'go');hold on;
           % plot(t+j-1,vars{i}.z{j}(1),'go');hold on;
