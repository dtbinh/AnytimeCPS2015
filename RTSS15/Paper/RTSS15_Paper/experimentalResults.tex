...
Figure \ref{fig:CostAndModes} shows a zoomed in view of how the tracking cost ($J_{true}$) evolves over time for RAMPC (with $\alpha=0$) and MPC (fixed mode 0). Also overlayed on tese two are the estimator modes being selected by RAMPC. Note, that RAMPC does better than MPC for these settings (lower tracking cost), as is also clear from table \ref{tbl:RAMPC_MPC_performance} and that there is dynamic switching of estimator modes at runtime with RAMPC leading to improved control performance.  
